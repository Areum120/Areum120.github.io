...
이슈를 파악하기 어려운 문제가 있습니다. 이에 고객 만족도를 높이고 서비스를 개선하여 비즈니스 적으로 활용하고자
솔루션을 요청했습니다. 

< 제안 >
솔루션1 : CIC 고객 상담 이력 대시보드 제작
CIC 고객 상담 이력을 NLU 분석한 대시보드를 구현합니다. 대시보드는 고객의 주요 이슈를 한눈에 파악하도록 시각화합니다. 상담 내용에서 주요 키워드 도출을 통해 고객의 불만사항과 니즈를 파악하도록 도와줍니다. 이는 의사결정 보조수단으로서 고객의 문제를 빠르게 해결합니다. 또한 서비스를 지속적으로 개선하여 고객 만족도를 높이는 것에 기여합니다.
<br><br>
제공 기능:
...
bert 모델은 transformer 계열의 모델인데, 
이 모델의 특징은 positional embedding을 고려합니다. 
그래서 단어의 의미를 파악할 때 단어로서 뿐만 아니라 그 단어가 문장내에서 어느 위치에 있는지, 
그리고 주변의 단어(조사 포함)는 어떤 단어가 있는지도 같이 파악하기 때문에 
발화자의 의도를 더 정확히 분류할 수 있습니다. <br>
그중에서도 KoBert 모델은 Skt에서 개발한 한국어 성능을 강화한 모델로 사용하였습니다.
약 3만건의 데이터를 학습하여 정확도 81% 모델을 개발하였으며 
세부 카테고리 분류에 따른 정의서 데이터와 추가적인 상담데이터를 통해 학습시키면
정확도를 개선할 거라 기대합니다.
...
진행하면서 가장 큰 어려웠던 점은 데이터를 늦게 받은 점이었습니다. 11월 26일 데모데이 발표일이었는데
3주 전 받게 되었습니다. 2개월이라는 PoC 기간 중 실제 데이터를 분석할 시간이 3주밖에 되지 않았으며
분석을 통해 인사이트를 도출하는 것과 개발을 동시에 진행해야 하는 지점, 
그 와중에 커뮤니케이션 비용으로 좀 더 분석과 개발에 투자할 시간이 부족한 게 
아쉬웠습니다. <br> 알고보니 외부의 아직 선정되지 않은 여러 기업들에게 NDA와 DPA 같은
...
PoC 이후에 5년 이상의 데이터를 확보하여 재구매 예측 모델을 이어서 개발하는 방안을 대안 제시하였습니다.
...
BDC, eCOM, EoT고객, NSC캠페인, bsi , CIC, bcc, Poi 등 BMW 내부에서 상요하는
업계용어로 이루어진 데이터라 분석에 어려움을 겪었습니다. 
그외 DSS계약일과 구매일, 인도일, 품의서상의 계약일, 차량 구매일 등 같은 구매 내용이라도
모두 칼럼으로 세세히 분류되어 있어 어떤 칼럼을 중요하게 봐야할 지 업계 기반의 지식을 숙지해야 했습니다.
다행히 수입자동차 업계 평균 20년 경력인
임원진 출신인 분들이 회사에 계셔서 각 용어에 대한 주석을 모두 여쭤보고 분석하였습니다.

**DB 설계 이슈**

data를 넣는 table간의 관계도를 설정하고 data를 어떻게 insert할 지
백엔드 API와 같이 만드는 부분이 어려웠습니다. data를 넣는 table 생성 및 ERD는
백엔드 디벨로퍼와 소통하여 만들었으나 이후에 테이블간의 외래키 설정으로 인해
데이터를 삭제하려니 참조 무결성 이슈가 있었습니다. 데이터를 delete 후 Insert할 때 
연결된 테이블에 각각 넣어야 하는 번거로움이 있어 
실제 서비스를 만들 때 외래키를 참조하는 것은 신중히 사용해야 한다는 것을 배웠습니다.


**카테고리 라벨 분류 이슈**

데이터 분석에서 어려웠던 점은 상담 카테고리 데이터 정의서가 없는 탓에 모든 상담 데이터를
형태소 분석기와 TDM을 통해 빈도수 높은 단어를 도출하여 카테고리를 전부 세분화하여 
라벨링을 해야 했습니다. 총 10만건의 데이터 중 일부 잘못 전화함, 끊김, Null 등의 불필요한 데이터를
삭제한 8만 6천 건의 데이터를 라벨링하기에는 어려우므로 LDA와 Bert모델을 사용하여 카테고리 라벨링을 학습 분류하고자 했고
이전에 같이 머신러닝 엔지니어 교육을 받은 친구를 파트타임으로 고용하여 함께 Bert모델을 개발하였습니다. 
...
메모리가 85% 꽉차 있는 문제가 있었습니다. 이후 t3.medium으로 업그레이드하자 8만 6천건의
