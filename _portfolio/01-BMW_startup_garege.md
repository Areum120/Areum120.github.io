...
또한 대시보드의 기간 별 키워드를 조회할 때 일부 9천건까지
데이터는 조회되지만 8만 6천건의 데이터 3개월을 넣었을 때 
로딩이 길어지면서 데이터가 조회되지않았습니다. AWS 서버용량이 t3.small로 9천건의 데이터만 넣어도
메모리가 85% 차지하고 있었습니다. <br> 이후 t3.medium으로 업그레이드하자 8만 6천건의
...
이에 8만 6천 건 데이터 조회 쿼리가 실행되는 속도가 30초에서 1초로 개선되었습니다.
